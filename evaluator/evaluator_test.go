...
func TestEvalIntegerExpression(t *testing.T) {
	tests := []struct {
		...
		{"5 + 5 + 5 + 5 - 10", 10},
		{"2 * 2 * 2 * 2 * 2", 32},
		{"-50 + 100 + -50", 0}, {"5 * 2 + 10", 20},
		{"5 + 2 * 10", 25}, {"20 + 2 * -10", 0},
		{"50 / 2 * 2 + 10", 60}, {"2 * (5 + 10)", 30},
...
func TestIfElseExpression(t *testing.T) {
	...
	tests := []struct {
		...
		{"if (true) { 10 }", 10},
		{"if (false) { 10 }", nil}, // this does not have an else block, so it returns nil
		{"if (1) { 10 }", 10},
		{"if (1 < 2) { 10 }", 10},
		{"if (1 > 2) { 10 }", nil},
...
func TestFunctionApplication(t *testing.T) {
	...
	tests := []struct {
		...
		{"let identity = fn(x) { return x; }; identity(5);", 5}, // this should return 5 and ignore 10
...
func TestReturnStatements(t *testing.T) {
	...
	tests := []struct {
		...
		{"return 10;", 10},
		{"return 10; 9;", 10}, // this should return 10 and ignore 9
		{"return 2 * 5; 9;", 10},
		{"9; return 2 * 5; 9;", 10}, // this should return 10 and ignore 9
		{`
		if (10 > 1) {
			if (10 > 1) {
				return 10;
